...
# For UPDATE: migrate root from mysql_native_password to caching_sha2_password (removed in MySQL 8.4)
	...
	if { [ "$MYSQL_SERVER_HOST" = "localhost" ] || [ "$MYSQL_SERVER_HOST" = "127.0.0.1" ]; } && \
	   ! mysql -h"$MYSQL_SERVER_HOST" -u"$MYSQL_SERVER_USER" -p"$MYSQL_SERVER_PASS" -e ";" >/dev/null 2>&1; then
		...
		mysql -uroot -e "FLUSH PRIVILEGES; ALTER USER '${MYSQL_SERVER_USER}'@'localhost' IDENTIFIED WITH caching_sha2_password BY '${MYSQL_SERVER_PASS}';"
		kill "$MYSQLD_TMP_PID" 2>/dev/null || true; wait "$MYSQLD_TMP_PID" 2>/dev/null || true
		systemctl start mysqld
		for i in $(seq 30); do mysql -h"$MYSQL_SERVER_HOST" -u"$MYSQL_SERVER_USER" -p"$MYSQL_SERVER_PASS" -e ";" >/dev/null 2>&1 && break; sleep 1; done
	fi
fi
